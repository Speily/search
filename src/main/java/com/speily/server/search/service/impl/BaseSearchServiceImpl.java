 ...
 * @Description: -elasticsearch 搜索引擎 基础实现 ElasticsearchTemplate
 */
public class BaseSearchServiceImpl<T> implements BaseSearchService<T> {
    ...
    /**
     * 高亮显示，返回分页,使用自带Pageable接口
     * @return
     */
    public org.springframework.data.domain.Page<Product> queryPageHit(int yuyu,int pageNo, int pageSize, String keyword, String indexName, String... fieldNames) {
        // 构造查询条件,使用标准分词器.

        Sort sort = new Sort(Sort.Direction.DESC, "id");//以id值为准 降序排列，ASC为升序
        Pageable pageable = PageRequest.of(0, 10, sort);//构造分页
        ...
        // 查询
        org.springframework.data.domain.Page<Product> productPage = elasticsearchTemplate.queryForPage(searchQuery, Product.class);
